...
plot_tiny <- data.frame(x = tiny$daynum, y = tiny$thg.wet.ng.g, 
                        fit_t = predictions[, "fit"], 
                        lwr_t = predictions[, "lwr"], 
                        upr_t = predictions[, "upr"])

# Plot the data with the polynomial fit and confidence intervals
ggplot(plot_tiny, aes(x = x, y = y)) +
  geom_point()+
  geom_line(aes(y = plot_tiny$fit_t, x=plot_tiny$x), color = "blue") +  # fitted polynomial line
  geom_ribbon(aes(ymin = lwr_t, ymax = upr_t), alpha = 0.2) +  # confidence interval
...
plot(orders_b, BIC_bs$BIC, xlab = "Polynomial Degree (k)")

bic_plot_b <- ggplot(BIC_bs, aes(x=orders_b, y=BIC))+
  geom_point()+
  geom_line()+
  ggtitle(lab= "13-20 mm Size Class")+
  xlab(label = "Polynomial Degree (k)")+
  ylab(label= "BIC")
...
ggplot(aes(orders_t, BICts))



bictp <- plot(BICts ~ orders_t)
bictp
...
shp_t_s <- shapiro.test(tiny$thg.wet.ng.g[tiny$season == "summer"])
shp_t_ef <- shapiro.test(tiny$thg.wet.ng.g[tiny$season == "early_fall"])
shp_t_lf <- shapiro.test(tiny$thg.wet.ng.g[tiny$season == "late_fall"])

# Create a data frame
shapiro_results_t <- data.frame(
  Season = c("Summer", "Early Fall", "Late Fall"),
  W = c(shp_t_s$statistic, shp_t_ef$statistic, shp_t_lf$statistic),
  p_value = c(shp_t_s$p.value, shp_t_ef$p.value, shp_t_lf$p.value)
)

# Print the table
print(shapiro_results_t)
